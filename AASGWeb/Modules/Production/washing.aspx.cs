...
namespace AASGWeb.Modules.Production
{
    public partial class washing : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    string _washingItemid = RouteData.Values["id"].ToString();
                    ItemId = App.Core.DataSecurity.Decryptdata(_washingItemid);
        ...
        private void PopulateData(string ItemId)
        {
            ...
            {
                hdnItemId.Value = ItemId;
                Int32 ProcessWashingID = Convert.ToInt32(ItemId);

                _objProcessWashingBL = new ProcessWashingBL();

                _objProcessWashing = _objProcessWashingBL.GetAllProcessWashingItems().Where(x => x.process_washing_id == Convert.ToInt32(ProcessWashingID)).FirstOrDefault();

                //_objProcessWashingBL = new ProcessWashingBL();
                //ProcessWashing _objProcessWashing = new ProcessWashing();
                //_lstProcessWashing = _objProcessWashingBL.GetAllProcessWashingItems().Where(x=>x.process_cutting_id==Convert.ToInt32(_ProcessWashingid)).ToList();
                if (_objProcessWashing != null)
                {
                    _objItemMasterBL = new ItemMasterBL();
                    _objItemMaster = _objItemMasterBL.GetItemMasterItemsByID(_objProcessWashing.item_master_id).FirstOrDefault();
                    if (_objItemMaster.item_master_id > 0)
                    {
                       
                    }

                    txtreceived.Text = _objProcessWashing.total_received.ToString();
                    txtbreakage.Text = _objProcessWashing.total_broken.ToString();
                    txtReject.Text = _objProcessWashing.total_reject.ToString();
                    txtFinalTransferred.Text = _objProcessWashing.total_pcs_transferred.ToString();
                    ddlSignature.SelectedValue = _objProcessWashing.signature.ToString();
                    txtRemarks.Text = _objProcessWashing.remarks;

                    if (_objProcessWashing.total_broken > 0)
                    {
                        bindBreakageReason(_objProcessWashing.total_broken);
                    }
                    else
                    {
                    ...
                    if (_objProcessWashing.total_reject > 0)
                    {
                        bindRejectReason(_objProcessWashing.total_reject);
                    }
                    else
                    {
                    ...
                    if (_objProcessWashing.total_pcs_transferred > 0)
                    {
                        txtFinalTransferred.Text = _objProcessWashing.total_pcs_transferred.ToString();
        ...
        {
            Int32 ProcessWashingID = Convert.ToInt32(hdnItemId.Value);
            ...
            if (_objProcessWashing != null)
            {
                ...
                if (_objProcessWashing.breakage_pcs_number == null)
                {

                }
                else
                {
                    string breakagePcsNumber = _objProcessWashing.breakage_pcs_number;
                    string lasttmPcsNumber = breakagePcsNumber.TrimEnd(',');
                    arrOfSelectionsPcsNumber = lasttmPcsNumber.Split(',');
                }
                if (_objProcessWashing.breakage_reason == null)
                {

                }
                else
                {
                    string breason = _objProcessWashing.breakage_reason;
        ...
        private void bindRejectReason(int total_reject)
        {
            ...
            if (_objProcessWashing != null)
            {
                ...
                if (_objProcessWashing.reject_pcs_number == null)
                {
                }
                else
                {
                    string rejectPcsNumber = _objProcessWashing.reject_pcs_number;
                    string lasttmPcsNumber = rejectPcsNumber.TrimEnd(',');
                    arrOfSelectionsRejectPcsNumber = lasttmPcsNumber.Split(',');
                }
                if (_objProcessWashing.reject_reason == null)
                {

                }
                else
                {
                    string rejectReason = _objProcessWashing.reject_reason;
        ...
        {
            _objProcessWashing = new ProcessWashing();
            _objProcessWashingBL = new ProcessWashingBL();
            try
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            _objProcessWashing = _objProcessWashingBL.GetAllProcessWashingItems().Where(x => x.process_washing_id == Convert.ToInt32(hdnItemId.Value)).FirstOrDefault();
                            if (_objProcessWashing.process_washing_id > 0)
                            {
                                _objProcessWashing = _objProcessWashingBL.UpdateProcessWashing(GetProcessWashingUpdateObject());
                                if (_objProcessWashing.process_washing_id > 0)
                                {
                                    //int itemMasterId = _objProcessWashing.item_master_id;
                                    //string batchNumber = _objProcessWashing.batch_number;
                                    ...
                                    if (_objProduction.production_id > 0)
                                    {
                                        ...
                                        if (_objProduction.production_id > 0)
                                        {
                                            ...
                                            string _heading = "Washing Submitted Successfully";
                                            string _heading_class = "text-success";
                                            string _activity = "Washing Submitted with Total Pcs Received:" + _objProcessWashing.total_received + ", Breakage:" + _objProcessWashing.total_broken + "" + ", Rejected:" + _objProcessWashing.total_reject + ", Total Pcs Transfer:" + _objProcessWashing.total_pcs_transferred;
                                           ...
                                           // ScriptManager.RegisterStartupScript(this, this.GetType(), "alertmessage", "showWashingProcessSuccess();", true);
                                        }
                                    }
        ...
        private ProcessWashing GetProcessWashingUpdateObject()
        {
         
            string breakReasonData = null;
            if (txtbreakage.Text == "0")
            {
                _objProcessWashing.breakage_reason = "";
            }
            else
            {
                ...
                _objProcessWashing.breakage_reason = totalBreakReasonData;
                ...
                _objProcessWashing.breakage_pcs_number = totalBreakagePcsNumber;
                _objProcessWashing.washing_status = App.Helper.Utils.OrderStatus.progress.ToString();
            }

            string rejectReasonData = null;
            if (txtReject.Text == "0")
            {
                _objProcessWashing.reject_reason = "";
            }
            else
            {
                ...
                _objProcessWashing.reject_reason = totalRejectReasonData;
                ...
                _objProcessWashing.reject_pcs_number = totalRejectPcsNumber;
            }


            _objProcessWashing.total_received = _objProcessWashing.total_received;
           // _objProcessWashing.total_received = Convert.ToInt32(txtreceived.Text);
            _objProcessWashing.total_broken = Convert.ToInt32(txtbreakage.Text);
            _objProcessWashing.total_reject = Convert.ToInt32(txtReject.Text);
            _objProcessWashing.total_pcs_transferred = Convert.ToInt32(txtFinalTransferred.Text);

            _objProcessWashing.signature = Convert.ToInt32(ddlSignature.SelectedValue);
            _objProcessWashing.washing_ended_on = System.DateTime.Now;
            _objProcessWashing.remarks = txtRemarks.Text;
            _objProcessWashing.washing_status = App.Helper.Utils.OrderStatus.complete.ToString();
