...
{
    public partial class grinding : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    string _grindingItemid = RouteData.Values["id"].ToString();
                    ItemId = App.Core.DataSecurity.Decryptdata(_grindingItemid);
        ...
        private void PopulateData(string ItemId)
        {
            try
            {
                ...
                _objProcessGrinding = _objProcessGrindingBL.GetAllProcessGrindingItems().Where(x => x.process_grinding_id == Convert.ToInt32(ProcessGrindingID)).FirstOrDefault();

                //_objProcessGrindingBL = new ProcessGrindingBL();
                //ProcessGrinding _objProcessGrinding = new ProcessGrinding();
                //_lstProcessGrinding = _objProcessGrindingBL.GetAllProcessGrindingItems().Where(x=>x.process_cutting_id==Convert.ToInt32(_ProcessGrindingid)).ToList();
                if (_objProcessGrinding != null)
                {
                    //btnUpdate.Visible = true;

                    //_objItemMasterBL = new ItemMasterBL();
                    //_objItemMaster = _objItemMasterBL.GetItemMasterItemsByID(_objProcessGrinding.item_master_id).FirstOrDefault();
                    //if (_objItemMaster.item_master_id > 0)
                    //{
                       
                    //}

                    //txtItemName.Text = _objProcessGrinding.item_brand + " " + _objProcessGrinding.item_model;
                    //txtthickness.Text = _objProcessGrinding.item_thickness;
                    //txtsheetSize.Text = _objProcessGrinding.item_sheet_size;
                   

                    txtreceived.Text = _objProcessGrinding.total_received.ToString();
                    txtReject.Text = _objProcessGrinding.total_reject.ToString();
                    txtbreakage.Text = _objProcessGrinding.total_broken.ToString();
                   
                    txtFinalTransferred.Text = _objProcessGrinding.total_pcs_transferred.ToString();
                    ddlSignature.SelectedValue = _objProcessGrinding.signature.ToString();

                    txtRemarks.Text = _objProcessGrinding.remarks;
                    if (_objProcessGrinding.total_broken > 0)
                    {
                        bindBreakageReason(_objProcessGrinding.total_broken);
                    }
                    else
                    {
                    ...
                    if (_objProcessGrinding.total_reject > 0)
                    {
                        bindRejectReason(_objProcessGrinding.total_reject);
                    }
                    else
                    {
                    ...
                    if (_objProcessGrinding.total_pcs_transferred > 0)
                    {
                        txtFinalTransferred.Text = _objProcessGrinding.total_pcs_transferred.ToString();
        ...
        private void bindBreakageReason(int total_broken)
        {
            ...
            if (_objProcessGrinding != null)
            {
                ...
                if (_objProcessGrinding.breakage_pcs_number == null)
                {

                }
                else
                {
                    string breakagePcsNumber = _objProcessGrinding.breakage_pcs_number;
                    string lasttmPcsNumber = breakagePcsNumber.TrimEnd(',');
                    arrOfSelectionsPcsNumber = lasttmPcsNumber.Split(',');
                }
                if (_objProcessGrinding.breakage_reason == null)
                {

                }
                else
                {
                    string breason = _objProcessGrinding.breakage_reason;
        ...
        private void bindRejectReason(int total_reject)
        {
            ...
            if (_objProcessGrinding != null)
            {
                ...
                if (_objProcessGrinding.reject_pcs_number == null)
                {
                }
                else
                {
                    string rejectPcsNumber = _objProcessGrinding.reject_pcs_number;
                    string lasttmPcsNumber = rejectPcsNumber.TrimEnd(',');
                    arrOfSelectionsRejectPcsNumber = lasttmPcsNumber.Split(',');
                }
                if (_objProcessGrinding.reject_reason == null)
                {

                }
                else
                {
                    string rejectReason = _objProcessGrinding.reject_reason;
        ...
        {
            _objProcessGrinding = new ProcessGrinding();
            _objProcessGrindingBL = new ProcessGrindingBL();
            try
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            _objProcessGrinding = _objProcessGrindingBL.GetAllProcessGrindingItems().Where(x => x.process_grinding_id == Convert.ToInt32(hdnItemId.Value)).FirstOrDefault();
                            if (_objProcessGrinding.process_grinding_id > 0)
                            {
                                _objProcessGrinding = _objProcessGrindingBL.UpdateProcessGrinding(GetProcessGrindingUpdateObject());
                                if (_objProcessGrinding.process_grinding_id > 0)
                                {
                                    //int itemMasterId = _objProcessGrinding.item_master_id;
                                    //string batchNumber = _objProcessGrinding.batch_number;
                                    ...
                                    if (_objProduction.production_id > 0)
                                    {
                                        ...
                                        if (_objProduction.production_id > 0)
                                        {
                                            ...
                                            string _heading = "Grinding Submitted Successfully";
                                            string _heading_class = "text-success";
                                            string _activity = "Cutting Submitted with Total Pcs Received:" + _objProcessGrinding.total_received + ", Breakage:" + _objProcessGrinding.total_broken + "" + ", Rejected:" + _objProcessGrinding.total_reject + ", Total Pcs Transfer:" + _objProcessGrinding.total_pcs_transferred;
                                           ...
                                           // ScriptManager.RegisterStartupScript(this, this.GetType(), "alertmessage", "showGrindingProcessSuccess();", true);
                                        }
        ...
        private ProcessGrinding GetProcessGrindingUpdateObject()
        {
            ...
            {
                _objProcessGrinding.breakage_reason = "";
            }
            else
            {
                ...
                _objProcessGrinding.breakage_reason = totalBreakReasonData;
                ...
                _objProcessGrinding.breakage_pcs_number = totalBreakagePcsNumber;
                _objProcessGrinding.grinding_status = App.Helper.Utils.OrderStatus.progress.ToString();
            ...
            if (txtReject.Text == "0")
            {
                _objProcessGrinding.reject_reason = "";
            }
            else
            {
                ...
                _objProcessGrinding.reject_reason = totalRejectReasonData;
                ...
                _objProcessGrinding.reject_pcs_number = totalRejectPcsNumber;
               
            }


            _objProcessGrinding.total_received = _objProcessGrinding.total_received;
           // _objProcessGrinding.total_received = Convert.ToInt32(txtreceived.Text);
            _objProcessGrinding.total_broken = Convert.ToInt32(txtbreakage.Text);
            _objProcessGrinding.total_reject = Convert.ToInt32(txtReject.Text);
            _objProcessGrinding.total_pcs_transferred = Convert.ToInt32(txtFinalTransferred.Text);

            _objProcessGrinding.signature = Convert.ToInt32(ddlSignature.SelectedValue);
            _objProcessGrinding.grinding_ended_on = System.DateTime.Now;
            _objProcessGrinding.remarks = txtRemarks.Text;
            _objProcessGrinding.grinding_status = App.Helper.Utils.OrderStatus.complete.ToString();
