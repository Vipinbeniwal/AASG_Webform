    ...
    public partial class packing : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    string _packingtemid = RouteData.Values["id"].ToString();
                    ItemId = App.Core.DataSecurity.Decryptdata(_packingtemid);
        ...
        private void PopulateData(string ItemId)
        {
            ...
            {
                hdnItemId.Value = ItemId;
                Int32 ProcessPackagingID = Convert.ToInt32(ItemId);

                _objProcessPackagingBL = new ProcessPackagingBL();

                _objProcessPackaging = _objProcessPackagingBL.GetAllProcessPackagingItems().Where(x => x.process_packaging_id == Convert.ToInt32(ProcessPackagingID)).FirstOrDefault();

                //_objProcessPackagingBL = new ProcessPackagingBL();
                //ProcessPackaging _objProcessPackaging = new ProcessPackaging();
                //_lstProcessPackaging = _objProcessPackagingBL.GetAllProcessPackagingItems().Where(x=>x.process_cutting_id==Convert.ToInt32(_ProcessPackagingid)).ToList();
                if (_objProcessPackaging != null)
                {
                    _objItemMasterBL = new ItemMasterBL();
                    _objItemMaster = _objItemMasterBL.GetItemMasterItemsByID(_objProcessPackaging.item_master_id).FirstOrDefault();
                    ...
                    txtreceived.Text = _objProcessPackaging.total_received.ToString();
                    txtbreakage.Text = _objProcessPackaging.total_broken.ToString();
                    txtReject.Text = _objProcessPackaging.total_reject.ToString();
                    txtFinalTransferred.Text = _objProcessPackaging.total_pcs_transferred.ToString();
                    ddlSignature.SelectedValue = _objProcessPackaging.signature.ToString();
                    txtRemarks.Text = _objProcessPackaging.remarks;

                    if (_objProcessPackaging.total_broken > 0)
                    {
                        bindBreakageReason(_objProcessPackaging.total_broken);
                    }
                    else
                    {
                    ...
                    if (_objProcessPackaging.total_reject > 0)
                    {
                        bindRejectReason(_objProcessPackaging.total_reject);
                    }
                    else
                    {
                    ...
                    if (_objProcessPackaging.total_pcs_transferred > 0)
                    {
                        txtFinalTransferred.Text = _objProcessPackaging.total_pcs_transferred.ToString();
        ...
        {
            Int32 ProcessPackagingID = Convert.ToInt32(hdnItemId.Value);
            ...
            if (_objProcessPackaging != null)
            {
                ...
                if (_objProcessPackaging.breakage_pcs_number == null)
                {

                }
                else
                {
                    string breakagePcsNumber = _objProcessPackaging.breakage_pcs_number;
                    string lasttmPcsNumber = breakagePcsNumber.TrimEnd(',');
                    arrOfSelectionsPcsNumber = lasttmPcsNumber.Split(',');
                }
                if (_objProcessPackaging.breakage_reason == null)
                {

                }
                else
                {
                    string breason = _objProcessPackaging.breakage_reason;
        ...
        private void bindRejectReason(int total_reject)
        {
            ...
            if (_objProcessPackaging != null)
            {
                ...
                if (_objProcessPackaging.reject_pcs_number == null)
                {
                }
                else
                {
                    string rejectPcsNumber = _objProcessPackaging.reject_pcs_number;
                    string lasttmPcsNumber = rejectPcsNumber.TrimEnd(',');
                    arrOfSelectionsRejectPcsNumber = lasttmPcsNumber.Split(',');
                }
                if (_objProcessPackaging.reject_reason == null)
                {

                }
                else
                {
                    string rejectReason = _objProcessPackaging.reject_reason;
        ...
        {
            _objProcessPackaging = new ProcessPackaging();
            _objProcessPackagingBL = new ProcessPackagingBL();
            try
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            _objProcessPackaging = _objProcessPackagingBL.GetAllProcessPackagingItems().Where(x => x.process_packaging_id == Convert.ToInt32(hdnItemId.Value)).FirstOrDefault();
                            if (_objProcessPackaging.process_packaging_id > 0)
                            {
                                _objProcessPackaging = _objProcessPackagingBL.UpdateProcessPackaging(GetProcessPackagingUpdateObject());
                                if (_objProcessPackaging.process_packaging_id > 0)
                                {
                                    //int itemMasterId = _objProcessPackaging.item_master_id;
                                    //string batchNumber = _objProcessPackaging.batch_number;
                                    ...
                                    if (_objProduction.production_id > 0)
                                    {
                                        ...
                                        if (_objProduction.production_id > 0)
                                        {
                                            ...
                                            string _heading = "Packing Submitted Successfully";
                                            string _heading_class = "text-success";
                                            string _activity = "Packing Submitted with Total Pcs Received:" + _objProcessPackaging.total_received + ", Breakage:" + _objProcessPackaging.total_broken + "" + ", Rejected:" + _objProcessPackaging.total_reject + ", Total Pcs Transfer:" + _objProcessPackaging.total_pcs_transferred;
                                            ...
                                            //ScriptManager.RegisterStartupScript(this, this.GetType(), "alertmessage", "showPackagingProcessSuccess();", true);
                                        }
        ...
        private ProcessPackaging GetProcessPackagingUpdateObject()
        {
            ...
            {
                _objProcessPackaging.breakage_reason = "";
            }
            else
            {
                ...
                _objProcessPackaging.breakage_reason = totalBreakReasonData;
                ...
                _objProcessPackaging.breakage_pcs_number = totalBreakagePcsNumber;
                _objProcessPackaging.packaging_status = App.Helper.Utils.OrderStatus.progress.ToString();
            }
            ...
            if (txtReject.Text == "0")
            {
                _objProcessPackaging.reject_reason = "";
            }
            else
            {
                ...
                _objProcessPackaging.reject_reason = totalRejectReasonData;
                ...
                _objProcessPackaging.reject_pcs_number = totalRejectPcsNumber;
            }


            _objProcessPackaging.total_received = _objProcessPackaging.total_received;
            // _objProcessPackaging.total_received = Convert.ToInt32(txtreceived.Text);
            _objProcessPackaging.total_broken = Convert.ToInt32(txtbreakage.Text);
            _objProcessPackaging.total_reject = Convert.ToInt32(txtReject.Text);
            _objProcessPackaging.total_pcs_transferred = Convert.ToInt32(txtFinalTransferred.Text);

            _objProcessPackaging.signature = Convert.ToInt32(ddlSignature.SelectedValue);
            _objProcessPackaging.packaging_ended_on = System.DateTime.Now;
            _objProcessPackaging.remarks = txtRemarks.Text;
            _objProcessPackaging.packaging_status = App.Helper.Utils.OrderStatus.complete.ToString();
